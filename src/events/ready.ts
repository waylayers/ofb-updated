...
export default new SelfbotEvent({
  ...
  async run(selfbot) {
    ...
    selfbot.logger.info(
      `You can pause the bot any time using "${selfbot.config.prefix}pause" and resume using "${selfbot.config.prefix}resume". You can also use these commands in here (in the command line), without including the prefix.\nThe bot will automatically pause whenever there is a captcha, so stay tuned in for notifications regarding captcha and solve them accordingly in order to not get banned.`
    );
    selfbot.updateRPC();
  },
});
